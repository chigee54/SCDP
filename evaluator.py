...
def compute_jer(ref: np.array, sys: np.array, cm: np.array, ref_idx: np.array, sys_idx: np.array,
                reverse: bool = False) -> np.array:
    """
    Code by Benedikt Bönninghoff <benedikt.boenninghoff@rub.de>
...
def main(solution, truth, output):
    ...
    task2_solutions = read_solution_files(os.path.join(args.predictions, 'result'))
    task2_truth = read_ground_truth_files(os.path.join(args.truth, 'dataset2/validation'))
    try:
        task2_f1 = compute_score_multiple_predictions(task2_truth, task2_solutions, 'paragraph-authors',
                                                      labels=[1, 2, 3, 4, 5])
    except KeyError as _:
        task2_f1 = None
    ...
    try:
        task2_jer, task2_der = compute_secondary_metrics(ref_list=task2_truth, sys_list=task2_solutions,
                                                         task_key='paragraph-authors', reverse=True)
    except KeyError as _:
        task2_jer = None
        task2_der = None
        print("No solution file found for one or more problems, please check the output. Exiting task 2.")
...
if __name__ == "__main__":
    ...
    solution = './output/dataset2'
    truth = './pan22'
    output = './output/dataset2'
    main(solution, truth, output)
